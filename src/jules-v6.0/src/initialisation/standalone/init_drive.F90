...
!DSM    ! Open the file
!DSM    OPEN(namelist_unit, FILE=file, STATUS='old', POSITION='rewind',           &
!DSM                        ACTION='read', IOSTAT = error, IOMSG = iomessage)
!DSM    IF ( error /= 0 )                                                         &
!DSM      CALL log_fatal(RoutineName,                                             &
!DSM                     "Error opening file " // TRIM(FILE) // " " //            &
!DSM                     "(IOSTAT=" // TRIM(to_string(error)) // " IOMSG=" //     &
!DSM                     TRIM(iomessage) // ")")
...
!DSM      IF ( error /= 0 )                                                       &
!DSM        CALL log_fatal(RoutineName,                                           &
!DSM                       "Error reading file name/time pair from file " //      &
!DSM                       "(IOSTAT=" // TRIM(to_string(error)) // " IOMSG=" //   &
!DSM                       TRIM(iomessage) // ")")
      ! Convert the file time as a string into a datetime object

    file_names(i)='xxx.nc'
    !write(file_time_str,'(a11,i2.2,a6)') '2010-01-03 ',i-1,':00:00'  !DSM
    file_time_str='2000-01-01 00:00:00'  !DSM apenas para ser um valor valido
!    print*,file_time_str
