...
CASE ( jules )
  ...
  IF ( soil_bgc_model == soil_model_rothc ) THEN
    DO l = 1, land_pts
      ...
      DO n = 1, dim_cslayer
        psparms%clay_soilt(l,1,n) = soil_clay_ij(i,j)
      END DO
    END DO
  END IF
  ...
  DO l = 1, land_pts
    DO n = 1, dim_cs1
      ...
      trifctltype%resp_s_soilt(l,1,1,n)  = 0.0
      !Can't copy from resp_s_gb_um as it's INTENT(OUT) so set to zero
      trifctltype%resp_s_acc_soilt(l,1,1,n) = resp_s_acc_gb_um(l,n)
    END DO
  END DO
#else
  l_dust_diag = l_dust
  ...
  DO l = 1, land_pts
    DO n = 1, dim_cs1
      resp_s_gb_um(l,n) = trifctltype%resp_s_soilt(l,1,1,n)
      resp_s_acc_gb_um(l,n) = trifctltype%resp_s_acc_soilt(l,1,1,n)
    END DO
  END DO
#endif
