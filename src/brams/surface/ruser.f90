...
veg_ndvic  =1.
veg_ndvif  =1.
print*,"changing NDVI to 1 -- temporary -- subroutine sfcinit_nofile_user" 
call flush(6)
!      ,veg_ndvif   (1,1,ipat)
!  This subroutine is the intended location for a user to customize the
...
subroutine change_soil_moisture_init(n1,n2,n3,mzg,mzs,npat,ifm   &
  ...
  if(mynum==1) then
      ...
      write(unit=22,fmt='(A,E12.6,A)') "===> changing the initial soil moisture by " ,fct, " - check ruser.f90 routine"
      close(unit=22)
  endif
...
!cycle
             call qwtk(soil_energy(k,i,j,ipat), soil_water(k,i,j,ipat)*1.e3,  &
                       slcpd(nsoil), tempk, fracliq)
              !print*,"sm2=",soil_water(k,i,j,ipat)
              soil_energy(k,i,j,ipat) = (tempk - 273.15)   &
                   * (slcpd(nsoil) + soil_water(k,i,j,ipat)  * 4.186e6)  &
                   + soil_water(k,i,j,ipat)  * 3.34e8
           enddo
        enddo
