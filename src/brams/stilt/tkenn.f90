...
! Developed by Marcos Longo (Lab. MASTER - Univ. Sao Paulo/EPS Harvard University)         !
...
subroutine nakanishi(m1,m2,m3,m4,ia,iz,ja,jz,jd,tkep,tket,vt3dd,vt3de,vt3dh,vt3di          &
   ...
   ! vctr33 -> q z dz                                                                      !
   ! vctr35 -> q^3                                                                          !
   ! vctr36 -> Ps+Pb=(q^3/L) x (Sm x Gm+Sh x Gh)                                                  !
   ! vctr37 -> epsilon=(q^3/B1 x L)                                                           !
   ! vctr38 -> 2e = q^2                                                                     !
   ! vt3dd  -> dU/dz at the u points                                                       !
   ! vt3de  -> dV/dz at the v points                                                       !
   ! vt3dh  -> Kh*rho0                                                                     !
   ! vt3di  -> (dU/dz)^2+(dV/dz)^2 (M^2) at the beginning, then Kq x rho0                       !
   ! vt3dj  -> g/theta_v d(THETA_v)/dz (N^2)                                                   !
   !---------------------------------------------------------------------------------------!
   ...
   do j=ja,jz
      do i=ia,iz
         ...
         do k=k2w,m1-1
            ...
            vctr38(k) = max(2.0 * tkep(k,i,j),2*tkmin)                ! q^2
            vctr35(k) = vctr38(k)*vctr30(k)                           ! q^3

            !------------------------------------------------------------------------------!
            !  Restrictions in M^2, just to avoid singularities (following Janjic 2001)     !
            !------------------------------------------------------------------------------!
         ...
         do k=k2w,m1-1
            ...
            ! tive of dimensionless square of Brunt Vaisala frequency) and GM (dimension-  !
            ...
            ! This is to guarantee that Cw, defined as <w^2>/q^2 < Rsl (about 0.14)          !
            !------------------------------------------------------------------------------!
         ...
         do k=k2w,m1-1
            ...
            !------------------------------------------------------------------------------!
            !     Case of growing turbulence, it must use the modified Level 2,5 model      !
            ! (Nakanishi and Niino, 2004), otherwise it just uses the original Level 2,5.   !
            !------------------------------------------------------------------------------!
         ...
         do k=k2w,m1-1
            ...
            !------------------------------------------------------------------------------!
            !----- Finding Ps+Pb-epsilon=(q^3/L) x (SmGm+ShGh-1/B1) --------------------------!
            ...
            !-----  Kq=L x q x Sq (vt3di). N&N (2004), proposed Sq=2*Sm instead of Sq=0.2 -----!
            vt3di(k,i,j)=2.*scr1(k,i,j) ! Since Km=L x q x Sm... 
         end do
