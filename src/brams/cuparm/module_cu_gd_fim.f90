...
!-srf
   ...
   REAL,         INTENT(IN   ) ::  tkmin,akmin
   REAL,         INTENT(IN   ) ::  cpor, p00,rgas
!-srf
...
!
     ...
     real,    dimension (its:ite,kts:kte,numc) ::  chem,outc,outcs
...
!----------------------------------------------------------------------
     ! for FIM GF version
     real,    dimension (its:ite,kts:kte) :: cnvwt ,zu,zd              
     real,    dimension (its:ite) :: edt   ,tau_ecmwf            
     real,    dimension (its:ite,5)         ::  forcing 
     real,    dimension (its:ite,kts:kte) :: hco,hcdo
     
!----------------------------------------------------------------------
   chem=0.0;outc=0.0;outcs=0.0; hco=0.0;hcdo=0.0
   k22=0 ; jmin=0
   forcing(:,:)=0.
   APRi_GR = 0.0
   APRi_W  = 0.0
   ...
   do j=jts,jte
    do i=its,ite
     xmb_shallow  (i,j) =0.0
     kbcon_shallow(i,j) =0.0
     ktop_shallow (i,j) =0.0
    enddo
   enddo
   tcrit=258.
...
!------------     
  ...
  IF(IDEEP_GF ==1 )then
     ...
     ENDIF
!--- for FIM GF version ---------
     cnvwt=0.0;zu=0.0;zd=0.0;edt=0.0;tau_ecmwf=0.0
     CALL CUP_gf(chem,outc,cnvwt,zu,zd,edt,kpbli,zws,dhdt,0,xmb,zo,&
           ...
           kbcon,ktop,cupclw,    &
           xfi_ens,pri_ens,XLANDi,forcing,        &
           ichoice,ipr,jpr,ierr,ierrc,    &
           beta,autoconv,aeroevap,numc,itf,jtf,ktf,training, &
...
!     CALL CUP_gf(kpbli,zws,dhdt,0,xmb,zo,outqc,j,AAEQ,T2d,Q2d,TER11,subm,TN,QO,PO,PRET,&
...
!           xlv,r_v,cp,g,ichoice,ipr_deep,jpr_deep,ierr,ierrc,ens4,&
!           beta,autoconv,aeroevap,itf,jtf,ktf,training,           &
!           use_excess,its,ite, jts,jte, kts,kte ,                 &
!           zustart_dp,zufinal_dp,zutop_dp,dicycle,entr_rate_deep,entrnew  &
!           !srf- for training
!	   ,iweight_GR    &
!	   ,iweight_W	  &
!	   ,iweight_MC    &
...
!	   !-srf- here iens is assumed equal to "1"
...
!
      DO I=its,itf
       DO K=kts,ktf
        qcheck(i,k)=q2d(i,k)+(outqs(i,k)+outqm(i,k)+subqm(i,k))*dt
       ENDDO
      ENDDO
      CALL neg_check('deep',j,outc,dt,qcheck,outq,outt,outu,outv,    &
...
!
   SUBROUTINE CUP_gf(chem,outc,cnvwt,zuo,zdo,edto,kpbl,zws,dhdt,imid,xmb_out, &
              zo,OUTQC,J,AAEQ,T,Q,Z1,sub_mas,   &
              ...
              xf_ens,pr_ens,xland,               &
              forcing,ichoice,ipr,jpr,ierr,ierrc,         &
              beta,autoconv,aeroevap,numc_X,itf,jtf,ktf,training,               &
!#if ( WRF_DFI_RADAR == 1 )
...
!#endif
   ...
   IMPLICIT NONE
     ...
     integer                                                           &
        ...
        its,ite, jts,jte, kts,kte,ipr,jpr,imid,numc_X
     integer, intent (in   )              ::                           &
  ...
  ! pre    = output precip
     ...
     real,    dimension (its:ite,kts:kte,numc_X)                              &
        ,intent (inout  )                   ::                           &
         chem,outc
     real,    dimension (its:ite,kts:kte,numc_X)   ::                           &
         chem_cup,chem_up,chem_down,dellac
     real,    dimension (its:ite)                                      &
        ,intent (inout  )                   ::                           &
        pre,xmb_out
     real,    dimension (its:ite,5 )                                      &
        ,intent (inout  )                   :: forcing
     integer,    dimension (its:ite)                                   &
        ,intent (inout  )                   ::                           &
        kbcon,ktop
     integer,    dimension (its:ite)                                   &
...
!
  ...
  ! dellaqc = change of qc per unit mass flux of cloud ensemble
        ...
        u_cup,v_cup,uc,vc,ucd,vcd,dellu,dellv,c1k
...
!srf- begin - DICYCLE
      ...
      integer :: nv,masscon,nvar
      integer:: numc= 0 ! local, dummy for turn off chem-transport
...
!
      ...
      enddo
       ...
       i=ipr
!!sms$ignore begin
!         write(12,*)i,'k,z(i,k),he(i,k),hes(i,k)'
!       do k=kts,ktf
...
!
      call rates_up_pdf('deep',ktop,ierr,po_cup,entr_rate_2d,hkbo,heo,heso_cup,zo_cup, &
                    kstabi,k22,kbcon,its,ite,itf,kts,kte,ktf,zuo,kpbl,ktopdby)
...
!-- formulation 2
         ...
         enddo
         kk=ktop(i)-maxloc(zuo(i,:),1)
         do k=maxloc(zuo(i,:),1)+1,ktop(i)
         !=> above maximum value zu -> change detrainment
           c1k(i,k)=c1+float((k-maxloc(zuo(i,:),1)))*(c1m-c1)/float(kk)
           dz=zo_cup(i,k)-zo_cup(i,k-1)
...
!!sms$ignore begin
!            write(13,*)ktop(i),j,'k,p_cup(i,k),heo_cup(i,k),heso_cup(i,k),hco(i,k)'
!            do k=kts,ktop(i)
!              write(13,*)k,p_cup(i,k),heo_cup(i,k),heso_cup(i,k),hco(i,k)
!            enddo
!!sms$ignore end
!         endif
         IF(ierr(I).eq.0)THEN
...
!             beta=.01
        ...
        dd_massdetro(i,:)=0.
        dd_massentro(i,:)=0.
           call get_zu_zd_pdf_fim(zuh2,"DOWN",ierr(i),kdet(i),jmin(i),zdo(i,:),kts,kte,ktf,beta,kpbl(i))
...
! initialize tracers if they exist
         do i=its,itf
         if(numc.gt.0 .and. ierr(i).eq.0)then
          do nv=1,numc
            do k=kts+1,ktf
...
!
      if(imid.eq.0)then
      call cup_up_moisture(c1k,'deep',ierr,zo_cup,qco,qrco,pwo,pwavo, &
      ...
      call cup_up_moisture(c1k,'shallow',ierr,zo_cup,qco,qrco,pwo,pwavo, &
           ccnclean,p_cup,kbcon,ktop,cd,dbyo,clw_all, &
           t_cup,qo,GAMMAo_cup,zuo,qeso_cup,k22,qo_cup,        &
           ZQEXEC,use_excess,ccn,rho,up_massentr,up_massdetr,psum,psumh,&
           autoconv,aeroevap,1,itf,jtf,ktf,j,ipr,jpr, &
...
!	!- multiply aa1_bl the " time-scale" - tau_bl
	  ...
	  !- version for real cloud-work function
          ...
          DO i=its,itf
            IF(ierr(I).eq.0)THEN
               hkbo_bl(i)=heo_cup_bl(i,k22(i)) ! +float(use_excess)*(xlv*zqexec(i)+cp*ztexec(i))
            endif ! ierr
          ENDDO
          DO k=kts,ktf
           do i=its,itf
...
!!sms$ignore begin
!            write(12,*)'zu(i,k),zd(i,k),pwo(i,k),pwdo(i,k)'
!          do k=1,ktop(i)+1
!            write(12,*)zu(i,k),zd(i,k),pwo(i,k),pwdo(i,k)
!          enddo
!!sms$ignore end
...
!            detup=up_massdetro(i,k)
            dz=zo_cup(i,k+1)-zo_cup(i,k)
	    dellaqc(i,k) = zuo(i,k)*c1k(i,k)*qrco(i,k)*dz/dp*g !detup*0.5*(qrco(i,k+1)+qrco(i,k)) *g/dp
     ...
     if (numc.gt.0)then
      ...
      do i=its,itf
        if(ierr(i).eq.0)then
            ...
            dellac(i,1,nv)=(edto(i)*zdo(i,2)*chem_down(i,2,nv)   &
                     -edto(i)*zdo(i,2)*chem_cup(i,2,nv))*g/dp
            do k=kts+1,ktop(i)
               dp=100.*(po_cup(i,k)-po_cup(i,k+1))
            ! these three are only used at or near mass detrainment and/or
            ! entrainment levels

               dellac(i,k,nv) =-(zuo(i,k+1)*(chem_up (i,k+1,nv)-chem_cup(i,k+1,nv) ) - &
                    zuo(i,k  )*(chem_up (i,k,nv  )-chem_cup(i,k,nv  ) ) )*g/dp &
                    +(zdo(i,k+1)*(chem_down(i,k+1,nv)-chem_cup(i,k+1,nv) ) - &
                    zdo(i,k  )*(chem_down(i,k,nv  )-chem_cup(i,k,nv  ) ))*g/dp*edto(i)
            enddo
...
!      if(j.eq.jpr)then
!               write(0,*)'xt',xl,'DELLAH(I,K),DELLAQ(I,K)
!      endif
!     if(j.eq.jpr) write(0,*)'DELLAS'
      do i=its,itf
         if(ierr(i).eq.0)then
!           if(J.eq.312772)write(13,*)'k,dellat(i,k),dellaq(i,k),XT(i,k),XHE(i,k)',edto(i),sigd(i),edtc
           do k=kts,ktf
...
!           if(J.eq.312772)write(13,444)k,dellat(i,k),dellaq(i,k),XT(i,k),XHE(i,k)
...
!
      call cup_up_aa0(xaa0,xz,xzu,xdby,GAMMA_CUP,xt_cup, &
           ...
           its,ite, jts,jte, kts,kte)
!sms$ignore begin
if(j.eq.312772)write(12,*)'xaa0,aa0,aa1 = ',xaa0(1),aa0(1),aa1(1)
!sms$ignore end
      do 200 nens=1,maxens
      do i=its,itf 
         if(ierr(i).eq.0)then
    ...
    do i=its,itf
     if(ierr(i).eq.0) then
      ...
      do k=kts,ktop(i)+1
       ...
       nvar=4 
       call setgradsvar(jl,k,nvar,dellaq(i,k)*86400.*xlv/cp*xmb(i), "dellq" ,'dellaq')

       nvar=5 
       call setgradsvar(jl,k,nvar,dellaqc(i,k)*86400.*xlv/cp*xmb(i),"dellqc" ,'dellaqc')
       ...
       nvar=9
       call setgradsvar(jl,k,nvar,outq(i,k)*86400.*xlv/cp,"outq" ,'outq K/s')

       nvar=10
       call setgradsvar(jl,k,nvar,outqc(i,k)*86400.*xlv/cp,"outqc" ,'outqc K/s')
...
!                  enddo
                   xff_ens3(6)=0.
                   do k=k22(i),kbcon(i)
                     xff_ens3(6)=xff_ens3(6)-omeg(i,k)/9.81
                   enddo
                   if(xff_ens3(6).le.0.)then
                     xff_ens3(6)=0.
                   else
                      xff_ens3(6)=xff_ens3(6)/(max(1.,float(kbcon(i)-k22(i)+1)))
                   endif
                   xff_ens3(5)=xff_ens3(6)
                   xff_ens3(4)=xff_ens3(6)
                   forcing(i,2)=xff_ens3(5)
...
!- more like Bechtold et al. (JAS 2014)
                ...
                forcing(i,5)=xff_dicycle
!               xff_ens3(1)=(AA1(i))/(60.*30.) !tau_ecmwf(i)
!               xff_ens3(2)=(AA1(i))/(60.*30.) !tau_ecmwf(i)
!               xff_ens3(3)=(AA1(i))/(60.*30.) !tau_ecmwf(i)
!-srf end  
!
!gtest
                if(icoic.eq.0)then
...
!                     if(xff0.gt.0.)then
                       ...
                       !srf  if(xff_ens3(13).gt.0)xf_ens(i,nall+13)=max(0.,-xff_ens3(13)/xk(ne))
!                      if(i.eq.ipr.and.j.eq.jpr)write(0,*)'xf_ens(nall+1)
!                      ',i,nall,xf_ens(i,nall+1)
                      else
                         xff_ens3(1)=0
                         xff_ens3(2)=0
                         xff_ens3(3)=0
...
!
                         ...
                         !srf xf_ens(i,nall+14)=max(0.,xff_ens3(14))
...
!                      ',xff_ens3(7),a1,xf_ens(i,nall+7)
                         ...
                         !srf a1=max(1.e-3,pr_ens(i,nall+15))
                         !srf xf_ens(i,nall+15)=max(0.,xff_ens3(15)/a1)
                         if(XK(ne).lt.0.)then
                            xf_ens(i,nall+10)=max(0.,-xff_ens3(10)/xk(ne))
                            xf_ens(i,nall+11)=max(0.,-xff_ens3(11)/xk(ne))
                            xf_ens(i,nall+12)=max(0.,-xff_ens3(12)/xk(ne))
                            xf_ens(i,nall+16)=max(0.,-xff_ens3(16)/xk(ne))
                         endif
!srf-begin
                        !srfIF(DICYCLE) THEN
                          if(XK(ne).lt.0.)then
                            xf_ens(i,nall+13)=max(0.,-xff_ens3(13)/xk(ne))
                            xf_ens(i,nall+14)=max(0.,-xff_ens3(14)/xk(ne))
                            xf_ens(i,nall+15)=max(0.,-xff_ens3(15)/xk(ne))
			  !ask georg if the line below should be here -OR- out of this if statement
			  xf_dicycle(i)      =  max(0.,-xff_dicycle /xk(ne))
                          else
                            xf_dicycle(i)      = 0.
                          endif
			  !xf_dicycle(i)      =       -xff_dicycle /xk(ne) !ask georg
			  !print*,"xf_dic1=",nall,i,icoic;call flush(6)
			  !print*,"xf_dic2=",xf_dicycle(i),xff_dicycle;call flush(6)
                        !srfENDIF
!srf-end
                      if(icoic.ge.1)then
                      closure_n(i)=0.
...
!          print *,'1',' adjusted massflux by factor ',i,j,k,qmem,qmem2,qmemf,dt
         ...
         if(qmem.lt.-thresh*names)then
           qmem2=-names*thresh/qmem
           qmemf=min(qmemf,qmem2)
      icheck=2
!
!
...
!-------------------------------------------------------
   SUBROUTINE cup_up_moisture(c1k,name,ierr,z_cup,qc,qrc,pw,pwav,     &
...
!
  ...
  !
     real,    dimension (its:ite,kts:kte)                              &
        ,intent (in   )                   ::                           &
        t_cup,p_cup,rho,q,zu,gamma_cup,qe_cup,c1k,                     &
...
!              QRCH=QES_cup(I,K)
              ...
              if(qc(i,k).gt.qrch)then
                DZ=Z_cup(i,K)-Z_cup(i,K-1)
                QRC(I,K)=(QC(I,K)-QRCH)/(1.+(c0+c1k(i,k))*DZ)
...
!--------------------------------------------------------------------
   ...
   SUBROUTINE CUP_gf_sh(chem,outc,cnvwt,zuo,zws,xmb_out,xmbs2,zo,OUTQC,J,AAEQ,T,Q,Z1,       &
              TN,QO,PO,P,OUTT,OUTQ,DTIME,PSUR,US,VS,                  &
              TCRIT,ztexec,zqexec,ccn,ccnclean,rho,dx,dhdt,           &
              cupclw,kpbl,kbcon,ktop,k22,         &   !-lxz
              tscl_kf,ichoice,ipr,jpr,ierr,ierrc,         &
              autoconv,numc,itf,jtf,ktf,               &
              use_excess,its,ite, jts,jte, kts,kte &
                                                )
   ...
   IMPLICIT NONE
     ...
     integer                                                           &
        ...
        its,ite, jts,jte, kts,kte,ipr,jpr,numc
     integer, intent (in   )              ::                           &
        j,ichoice
  !
  ! 
  ...
  !
     ...
     real, dimension (its:ite)                                         &
        ,intent (in   )                   ::                           &
        zws,ztexec,zqexec,ccn,Z1,PSUR,AAEQ
...
!
  ...
  ! edt     = epsilon
     ...
     real                                 ::                           &
      ...
      massfld,dh,cap_maxs,trash,trash2,trashq,frh,xlamdd,fsum
     ...
     integer :: nv,icount,tun_lim,jprnt,k1,k2,kbegzu,kfinalzu,kstart,jmini
     logical :: keep_going
...
!
      ...
      call rates_up_pdf('shallow',ktop,ierr,po_cup,entr_rate_2d,hkbo,heo,heso_cup,zo_cup, &
                    kstabi,k22,kbcon,its,ite,itf,kts,kte,ktf,zuo,kpbl,ktopx)
      do i=its,itf
         if(ierr(i).eq.0)then
...
!          write(0,*)k,dz,zuo(i,k-1),i
         ...
         do k=k22(i)+1,ktop(i)
          ...
          if(qco(i,k)>=trash) then
              DZ=Z_cup(i,K)-Z_cup(i,K-1)
              ! cloud liquid water
              qrco(i,k)= (qco(i,k)-trash)/(1.+c1_shal*dz)
              ! cloud water vapor 
...
412    continue
       if( name == 'deep' ) then
          if(kfinalzu.le.kbcon(i)+2)then
           ...
           call get_zu_zd_pdf_fim(zuh2,"UP",ierr(i),k22(i),kfinalzu,zuo(i,kts:kte),kts,kte,ktf,0.,kpbl(i))
          endif
       elseif ( name == 'shallow' ) then
          if(kfinalzu.le.kbcon(i)+1)then
           ...
           call get_zu_zd_pdf_fim(zuh2,"SH3",ierr(i),k22(i),kfinalzu,zuo(i,kts:kte),kts,kte,ktf,0.,kpbl(i))
           kbegin=0
...
!-------------------------------------------------------------------------
subroutine get_zu_zd_pdf_fim(zuh2,draft,ierr,kb,kt,zu,kts,kte,ktf,max_mass,kpbli)
...
!   zu(k+1)=FZU*kratio**(alpha-1.0) * (1.0-kratio)**(beta-1.0)
! enddo
  do k=kts,min(kt+1,ktf)
      kratio= float(k)/float(kt+1)
      zuh(k) = FZU*kratio**(alpha-1.0) * (1.0-kratio)**(beta-1.0)
