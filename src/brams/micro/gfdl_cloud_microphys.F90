...
! =======================================================================
! BRAMS version
! in subroutine gfdl_cloud_microphys_init ()!
!0) comment the lines (63 and 64 below)
!    !use fms_mod,             only: write_version_number, open_namelist_file, &
!    !                               check_nml_error, close_file, file_exist
!1)   include: integer :: nlunit = 4
!2)   include: close(nlunit)
!3)   comment the 2 lines below
!!srf        ierr = check_nml_error(ios,'gfdl_cloud_microphysics_nml')
!!srf        call close_file(nlunit)
!!srf
!!srf character (len = 64) :: fn_nml = 'input.nml'
!     character (len = 64) :: fn_nml = 'gfdl_cloud_microphysics_nml'
! =======================================================================
...
use node_mod, only: mynum
    ...
    real, parameter :: qmin = 1.0E-12 ! min q for computing precip type
    real, parameter :: t0c  = 2.7315e+2 ! temp at 0C (K) 
    !--srf
    real, parameter :: n0r = 8.0e6, n0s = 3.0e6, n0g = 4.0e6
    real, parameter :: rhow = 1.0e3, rhos = 1.0e2, rhog = 4.0e2
...
subroutine gfdl_cloud_microphys_driver (qv, ql, qr, qi, qs, qg, qa, qn,   &
        ...
        iis, iie, jjs, jje, kks, kke, ktop, kbot,re_cloud, re_ice, re_snow)
    ...
    !rsun 
    real, intent (inout), dimension (:, :, :) :: re_cloud, re_ice, re_snow
    ...
    real :: allmax
    real, dimension (ktop:kbot) :: qvz, qlz, qrz, qiz, qsz,  qaz,t1d
    real, dimension (ktop:kbot) :: dp0, dp1, dz0, dz1, qa0, den0
...
!################################    
    if(firsttime == 0 .and. mynum == 1) then 
        firsttime=1
        print*,"========================================================================"
        print*,"GFDL parameters:"
        print*,"mp_time, t_min, t_sub, tau_r2g, tau_smlt, tau_g2r, dw_land, dw_ocean", &
        mp_time, t_min, t_sub, tau_r2g, tau_smlt, tau_g2r, dw_land, dw_ocean 

        print*,"vi_fac, vr_fac, vs_fac, vg_fac, ql_mlt, do_qa, fix_negative, vi_max",  &
        vi_fac, vr_fac, vs_fac, vg_fac, ql_mlt, do_qa, fix_negative, vi_max

        print*,"vs_max, vg_max, vr_max, qs_mlt, qs0_crt, qi_gen, ql0_max, qi0_max",    &
        vs_max, vg_max, vr_max, qs_mlt, qs0_crt, qi_gen, ql0_max, qi0_max

        print*,"qi0_crt, qr0_crt, fast_sat_adj, rh_inc, rh_ins, rh_inr, const_vi",     &
        qi0_crt, qr0_crt, fast_sat_adj, rh_inc, rh_ins, rh_inr, const_vi

        print*,"const_vs, const_vg, const_vr, use_ccn, rthresh, ccn_l, ccn_o, qc_crt", &
        const_vs, const_vg, const_vr, use_ccn, rthresh, ccn_l, ccn_o, qc_crt

        print*,"tau_g2v, tau_v2g, sat_adj0, c_piacr, tau_imlt, tau_v2l, tau_l2v",      &
        tau_g2v, tau_v2g, sat_adj0, c_piacr, tau_imlt, tau_v2l, tau_l2v

        print*,"tau_i2s, tau_l2r, qi_lim, ql_gen, c_paut, c_psaci, c_pgacs",           &
        tau_i2s, tau_l2r, qi_lim, ql_gen, c_paut, c_psaci, c_pgacs
	
        print*,"z_slope_liq, z_slope_ice, prog_ccn, c_cracw, alin, clin, tice",        &
        z_slope_liq, z_slope_ice, prog_ccn, c_cracw, alin, clin, tice

        print*,"rad_snow, rad_graupel, rad_rain, cld_min, use_ppm, mono_prof",         &
        rad_snow, rad_graupel, rad_rain, cld_min, use_ppm, mono_prof

        print*,"do_sedi_heat, sedi_transport, do_sedi_w, de_ice, icloud_f, irain_f, mp_print", &
        do_sedi_heat, sedi_transport, do_sedi_w, de_ice, icloud_f, irain_f, mp_print
        print*,"========================================================================"
...
!################################    
    ...
    do j = js, je
        do i = is, ie
            !-- srf preventing neg precipitations
            rain    (i, j) = max(0., rain    (i, j) * convt)
            snow    (i, j) = max(0., snow    (i, j) * convt)
            ice     (i, j) = max(0., ice     (i, j) * convt)
            graupel (i, j) = max(0., graupel (i, j) * convt)
...
!           p1 (k) = den0 (k) * rdgas * t0 (k) ! dry air pressure
        enddo 
        islmski = nint(land(i,j)) ! 1=land,0=ocean
        !print*,"islmski",islmski 
       
        call effectRad(t1d, qlz, qiz, qsz, den0,               &    
                       qmin, t0c, re_qc, re_qi, re_qs,         &    
                       islmski,ktop, kbot, i, land(i,j))
        do k=kbot,ktop,-1
    ...
    !---------------------------------------------------
    !srf: for future use for radar reflectivity calculation
    !if(lradar) then
    ...
    do i = is, ie
        ...
        do n = 1, ntimes
            ...
            call icloud (ktop, kbot, tz, p1, qvz, qlz, qrz, qiz, qsz, qgz, dp1, den, &
                denfac, vtsz, vtgz, vtrz, qaz, rh_adj, rh_rain, dts, h_var, cnv_fraction(i))
...
subroutine icloud (ktop, kbot, tzk, p1, qvk, qlk, qrk, qik, qsk, qgk, dp1, &
        den, denfac, vts, vtg, vtr, qak, rh_adj, rh_rain, dts, h_var, cnv_fraction)
    ...
    real, intent (in) :: rh_adj, rh_rain, dts, h_var, cnv_fraction
...
! GEOS ! WMP impose CALIPSO ice polynomial from 0 C to -40 C on qi_crt  
            qi_crt = ice_fraction(tzk(k),cnv_fraction) * qi_gen / den (k)
...
! GEOS ! WMP impose CALIPSO ice polynomial from 0 C to -40 C on qi0_crt  
                qim = ice_fraction(tz,cnv_fraction) * qi0_crt / den (k)
        ! WMP Use h_var to add some resolution dependence on critical qi
        ! WMP   qim = (h_var**3) * qi0_crt / den (k)
    ...
    call subgrid_z_proc (ktop, kbot, p1, den, denfac, dts, rh_adj, tzk, qvk, &
        qlk, qrk, qik, qsk, qgk, qak, h_var, rh_rain, cnv_fraction)
...
subroutine subgrid_z_proc (ktop, kbot, p1, den, denfac, dts, rh_adj, tz, qv, &
    ql, qr, qi, qs, qg, qa, h_var, rh_rain, cnv_fraction)
    ...
    real, intent (in) :: dts, rh_adj, h_var, rh_rain, cnv_fraction
...
! GEOS ! WMP impose CALIPSO ice polynomial from 0 C to -40 C 
            rqi = ice_fraction(tin,cnv_fraction)
         ...
         if (qpz > qrmin) then
             ...
             ! top-hat
               ...
               elseif (qstar < q_plus .and. q_cond (k) > qc_crt) then
                 qa (k) = max(0.0, min(1., (q_plus - qstar) / (dq + dq) )) ! partial cloud cover
               elseif (qstar .le. q_minus) then
                 qa (k) = 1. ! air fully saturated; 100 % cloud cover
               endif
             endif
...
!   character (len = 64) :: fn_nml = 'input.nml'
    character (len = 64) :: fn_nml = './namelists/gfdl_cloud_microphysics_nml'
...
!#else
    ...
    !print*,"Does ",trim(fn_nml), "exist?", exists
    if (.not. exists) then
        write (6, *) 'gfdl - mp :: namelist file: ', trim (fn_nml), ' does not exist'
        stop
    else
      ! Read Main namelist
        !print*,"GFDL MP reading ",trim(fn_nml)
...
!    endif
   ...
   !endif
    ...
    !-srf for future use for computing radar reflectivity
    return
...
real function new_ice_condensate(tk, cnv_fraction, qlk, qik)

     real, intent(in) :: tk, cnv_fraction, qlk, qik
     real :: ptc, ifrac

     ifrac = ice_fraction(tk,cnv_fraction)
     if (qlk > qcmin) then
...
real function new_liq_condensate(tk, cnv_fraction, qlk, qik)

     real, intent(in) :: tk, cnv_fraction, qlk, qik
     real :: lfrac

     lfrac = 1.0 - ice_fraction(tk,cnv_fraction)
     if (qik > qcmin) then
...
!WMP mpoly = calipso_ice_polynomial(tk)
!WMP ice_fraction = (mpoly**4)*cnv_fraction + mpoly*(1.0-cnv_fraction)
...
!-----------------------------------------------------------------------
subroutine effectRad (t, qc, qi, qs, rho, qmin, t0c,        &
                     re_qc, re_qi, re_qs, islmski, kts, kte, ii,land)
...
!..Sub arguments
      integer, intent(in) :: kts, kte, ii
      real, intent(in) :: qmin,land
      real, intent(in) :: t0c
      integer, intent(in) :: islmski
